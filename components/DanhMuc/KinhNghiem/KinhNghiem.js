...
const buttonArray = ['Tất cả kinh nghiệm', 'Tạo mới kinh nghiệm'];
function KinhNghiemComponent(props) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            typeAction === 'EDIT'
                ? await updateKinhNghiemLamViec(idAction, dataBuild)
                : await createNewKinhNghiemLamViec(dataBuild);
            fetch();
            setTen('');
            ...
            swalert
                .fire({
                    ...
                    icon: 'warning',
                    text: 'Bạn đã tạo thành công kinh nghiệm làm việc   ',
    ...
    const handlePerformAction = ({ item, type }) => {
        ...
        if (type === 'EDIT') {
            ...
            swalert
                ...
                .then(async (result) => {
                    if (result.isConfirmed) {
                        await deleteKinhNghiemLamViec(item.id);
                        fetch();
                    }
    ...
    return (
        <div className={cx('wp')}>
            ...
            <SupperComponents
                titleAll="Tất cả kinh nghiệm làm việc"
                titleAdd="Thêm mới kinh nghiệm làm việc"
                data={data}
            ...
            >
                <div className={cx('item')}>
                    <label htmlFor="hoten">Kinh nghiệm làm việc</label>
                    <input
                        onChange={(e) => setTen(e.target.value)}
                        value={ten}
                        className="form-control"
                        id="hoten"
                        placeholder="eg: 1 năm kinh nghiệm, 2 năm kinh nghiệm...."
                        required
                    />
                </div>
                <div>
...
KinhNghiemComponent.propTypes = {};

export default KinhNghiemComponent;
